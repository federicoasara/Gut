...
class TimedSignaler:
	...
	func _on_timer_timeout():
		print('emitting signal ', Time.get_ticks_msec())
...
class TestYieldFor:
	...
	func test_new_yield():
		...
		assert_gt(_counter.time, 1.9)

	func test_passing_assert_ends_yield():
		await yield_for(0.5).timeout
		pass_test('yield should stop for this.')
		assert_gt(_counter.time, .49)
...
class TestYieldTo:
	...
	func test_can_yield_to_signal():
		...
		print(signaler.the_signal.get_connections())
		await gut.timeout
		print(Time.get_ticks_msec())
		assert_gt(_counter.time, .49)
		print(Time.get_ticks_msec())
